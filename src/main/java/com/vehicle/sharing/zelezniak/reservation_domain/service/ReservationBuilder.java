...
class ReservationBuilder {
    ...
    Reservation buildReservation(
        ...
        Reservation basic = request.getReservation();
        Reservation reservation = basic.toBuilder()
                .vehicles(handleGetActiveVehicles(
                        request.getVehiclesIds()))
                .client(clientService.findById(
                        request.getClientId()))
                .reservationStatus(Reservation.ReservationStatus.ACTIVE)
                .build();
        calculateAndSetEstimatedCost(reservation);
        return reservation;
    }

    private Set<Vehicle> handleGetActiveVehicles(
            Set<Long> vehiclesIds) {
        Collection<Vehicle> vehiclesByIDs = vehicleService.findVehiclesByIDs(
                vehiclesIds);
        Set<Vehicle> vehicles = new HashSet<>(vehiclesByIDs);
    ...
    private void calculateAndSetEstimatedCost(Reservation reservation) {
        Money c = reservationCalculator.calculateCost(reservation);
        reservation.setEstimatedCost(c);
    }
}
